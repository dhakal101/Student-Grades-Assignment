// Math Object Extentions

Math.randomDec = function(low, high) {
    // Return a random decimal low (inclusive) high (exclusive)
    return Math.random() * (high - low) + low;
}

Math.randomInt = function(low, high) {
    // Return a random integer low (inclusive) high (exclusive)
    return Math.floor(Math.randomDec(low, high));
}

Math.randomTo = function(num, numPlaces) {
